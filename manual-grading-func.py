...
validate_dirs(dir)
solutions = load_f_solutions(dir['sol'])
tests = load_f_tests(dir['test'])
file_sol = load_solutions(dir['sol_out'])
verbose = load_f_solutions(dir['sol'])
target_letter = ''
...
for lab in os.listdir(f"./{dir['sub']}"):
    ...
    try:
        ...
        repeat = False
        while temp_grade == 'r':
          grade_script(lab, dir['sub'], tests, solutions, output_only, verbose, modify=modify, methods=methods, repeat=repeat)
          if check_file:
              grade = file_check(file_info[0], id, file_sol, verbose)
          temp_grade = 'h'
          repeat = False
          ...
          while temp_grade == 'h':
            ...
            if temp_grade == '':
              ...
              target_letter = input('Enter first letter of last name: ')
            elif temp_grade == 'o':
